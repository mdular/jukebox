...
- a one-shot stdin replay confirms playback start once the configured target device is visible to Spotify
...
- If `target_visible` is `false`, the service should still remain up after boot. A replay failure in that state is a playback-target visibility problem, not a service-readiness failure.
...
6. If `jukebox` is not yet visible in the device snapshot, the service should still stay up and wait for scans instead of restart-looping.
7. A fresh physical card scan should still trigger playback once the receiver becomes visible to Spotify.
