...
class CommandSystemHelpers:
    ...
    def status(self) -> dict[str, bool]:
        payload = self._run_json(self.wifi_helper_command, "status")
        return {
    ...
    def start_setup_ap(self) -> tuple[bool, str]:
        return self._run_status(self.wifi_helper_command, "start-ap")
    ...
    def reset_wifi(self) -> tuple[bool, str]:
        return self._run_status(self.wifi_helper_command, "reset-client")

    def start_auth(self) -> dict[str, object]:
        return self._run_json(self.spotifyd_auth_helper_command, "start")

    def request_shutdown(self, *, reason: str) -> tuple[bool, str]:
        return self._run_status(self.shutdown_helper_command, "--reason", reason)
